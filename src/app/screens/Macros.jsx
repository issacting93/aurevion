...
export function MacroTargetsContent() {
  ...
  return (
    <div style={{ flex: 1, padding: '20px 24px 40px', overflowY: 'auto' }}>
      ...
      <FMono color={Color.mute}>DAILY · −480 DEFICIT · 16 WK PLAN</FMono>

      <div style={{ marginTop: 32, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
        <FSurface accent={Color.accent} style={{ padding: '12px 14px', borderRadius: 10 }}>
          <FLabel mb={4} color={Color.accent}>Protein</FLabel>
          <FNum size={24} weight={300} unit="g">147</FNum>
          <div style={{ marginTop: 8 }}>
            <div style={{ height: 3, background: Color.accent, borderRadius: Radius.full }}/>
          </div>
        </FSurface>
        <FSurface style={{ padding: '12px 14px', borderRadius: 10 }}>
          <FLabel mb={4}>Carbs</FLabel>
          <FNum size={24} weight={300} unit="g">160</FNum>
          <div style={{ marginTop: 8 }}>
            <div style={{ height: 3, background: 'rgba(255,255,255,0.15)', borderRadius: Radius.full }}/>
          </div>
        </FSurface>
        <FSurface style={{ padding: '12px 14px', borderRadius: 10 }}>
          ...
          <div style={{ marginTop: 8 }}>
            <div style={{ height: 3, background: 'rgba(255,255,255,0.10)', borderRadius: Radius.full }}/>
          </div>
        </FSurface>
      </div>
      ...
      <FSection label="This week's plan" mt={40} mb={12}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
          ...
          <FMono color={Color.mute}>3 BATCHES</FMono>
        </div>
      </FSection>
        ...
        {days.map((d, i) => {
          ...
          return (
            <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: Space[1], alignItems: 'center' }}>
              <FMono color={Color.mute} size={9}>{d}</FMono>
              <div style={{
                ...
                <span style={{ fontFamily: Font.mono, fontSize: 9, color: filled ? Color.accentText : Color.mute, fontWeight: 600 }}>
                  {filled ? '2' : '—'}
                </span>
              </div>
            </div>
      ...
      <FSurface style={{ marginTop: 40, padding: 16, borderRadius: Radius.lg }}>
      ...
      <div style={{ marginTop: 32 }}>
        <FListRow title="Review meals" trailing={<FIcon path={ICONS.fwd} size={18} color={Color.accent} stroke={2} />} onClick={() => {}} style={{ borderTop: `1px solid ${Color.border}` }} />
      </div>
  ...
  return (
    <Phone label="Meal queue" group="MACROS">
      <FNavBar
        ...
        leading={<FIcon path={ICONS.back} size={20} color={Color.text}/>}
        trailing={<FIcon path={ICONS.swap} size={20} color={Color.text}/>}
      />
      <div style={{ flex: 1, padding: '20px 24px 40px', overflowY: 'auto' }}>
        ...
        <div style={{ marginTop: 32, display: 'flex', flexDirection: 'column', gap: Space[3] }}>
          {meals.map((m, i) => (
        ...
        <FSurface style={{ marginTop: 32, padding: 16, borderRadius: Radius.lg }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              ...
              <FMono color={Color.mute} size={10}>{deltaVsTarget >= 0 ? '+' : ''}{deltaVsTarget} vs target</FMono>
            </div>
            <button
              onClick={swapOne}
              data-stay="true"
        ...
        <div style={{ marginTop: 32, display: 'flex', gap: Space[3] }}>
          ...
          <FListRow title="Build shopping list" trailing={<FIcon path={ICONS.fwd} size={18} color={Color.accent} stroke={2} />} onClick={() => {}} style={{ flex: 1.2, borderTop: `1px solid ${Color.border}` }} />
        </div>
      </div>
      <FTabBar active={1}/>
    </Phone>
  ...
  return (
    <Phone label="Shopping" group="MACROS">
      <FNavBar
        title="Shopping list"
        leading={<FIcon path={ICONS.back} size={20} color={Color.text}/>}
        trailing={<FIcon path={ICONS.filter} size={20} color={Color.text}/>}
      />
      <div style={{ flex: 1, padding: '20px 24px 40px', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          <div>
            <FLabel>To buy</FLabel>
            <FNum size={64} weight={200}>{remaining} <span style={{ fontSize: 18, color: Color.mute, fontFamily: Font.mono }}>items</span></FNum>
          </div>
          <FMono color={Color.green}>−{owned} ALREADY HOME</FMono>
        </div>
